...
if __name__ == "__main__":
    ...
    messages_to_player("Select dictionary")
    for n in range(len(DICTIONARIES)):
        messages_to_player("{0} - {1}".format(str(n), DICTIONARIES[n]['name']))
    selected_dictionary = int(raw_input('Which dictionary you want? Provide number: '))
    word_dictionary = read_file_with_words(DICTIONARIES[selected_dictionary]['file'])
    ...
    while play_on:
        ...
        wrong_guess = []
        word_to_guess = select_new_random_word(words=word_dictionary, already_done=words_done)
        ...
        if all_words_done(word_dictionary, words_done):
